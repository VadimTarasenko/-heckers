...
public class Move_w : MonoBehaviour {
	...
	void OnMouseDown() 
    {
        ...
        if (target.GetComponent<GameProcess> ().play1) {
			if (is_checker_active == true) {
                ...
                } else if (is_bChacker_in_square(active_chacker.transform.position.x + 2, active_chacker.transform.position.y + 2) 
                    && move_access(active_chacker.transform.position.x + 4, active_chacker.transform.position.y + 4)) {
				...
				if (is_bChacker_in_square(active_chacker.transform.position.x - 2, active_chacker.transform.position.y - 2) 
                    && move_access(active_chacker.transform.position.x - 4, active_chacker.transform.position.y - 4)) {
                    create_square(active_chacker.transform.position.x - 4, active_chacker.transform.position.y - 4, true, false);
                    find_rmChecker_position(active_chacker.transform.position.x - 2, active_chacker.transform.position.y - 2);
				} else if (is_bChacker_in_square(active_chacker.transform.position.x + 2, active_chacker.transform.position.y - 2) 
    ...
    public bool need_beat()
    {
        ...
        {
            if (is_bChacker_in_square(i.transform.position.x + 2, i.transform.position.y + 2) && move_access(i.transform.position.x + 4, i.transform.position.y + 4) && i.active)
            {
                need_beat = true; break;
            }
            else if (is_bChacker_in_square(i.transform.position.x - 2, i.transform.position.y + 2) && move_access(i.transform.position.x - 4, i.transform.position.y + 4) && i.active)
            {
                need_beat = true; break;
            }
            else if (is_bChacker_in_square(i.transform.position.x + 2, i.transform.position.y - 2) && move_access(i.transform.position.x + 4, i.transform.position.y - 4) && i.active)
    ...
    public bool damka_need_beat()
    {
        foreach(GameObject i in checker_white)
        {
            if(i.GetComponent<Move_w>().is_damka)
            {
